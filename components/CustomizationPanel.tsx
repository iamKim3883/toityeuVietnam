...
const SegmentedControl = <T extends string | number,>({ options, selectedValue, onChange, label }: {
    ...
    <div>
        <h3 className="text-md font-semibold text-red-100 mb-3">{label}</h3>
        <div className="flex w-full bg-red-800 p-1 rounded-xl">
            {options.map((option) => (
                <button
                    key={option.value.toString()}
                    onClick={() => onChange(option.value)}
                    className={`flex-1 text-center text-sm font-medium py-2 rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-red-800 focus:ring-red-400
...
const CustomizationPanel: React.FC<CustomizationPanelProps> = ({
  ...
  return (
    <div className="bg-red-900/50 p-6 rounded-2xl shadow-md">
      <h2 className="text-lg font-bold text-red-100 mb-4">4. Tuỳ chỉnh</h2>
